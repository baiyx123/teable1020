...
export const drawCells = (
  ...
  // Get cell save status function (imported dynamically to avoid circular deps)
  const getCellSaveState = (recordId?: string, fieldId?: string) => {
  ...
  const getCellStroke = (cellProps: { recordId?: string; fieldId?: string }) => {
    const saveState = getCellSaveState(cellProps.recordId, cellProps.fieldId);
    switch (saveState) {
      case 'saving':
        return 'rgb(96, 165, 250)'; // 蓝色
      case 'saved':
        return 'rgb(74, 222, 128)'; // 绿色
      case 'error':
        return 'rgb(248, 113, 113)'; // 红色
      default:
        return cellLineColor; // 默认颜色
    }
  };
