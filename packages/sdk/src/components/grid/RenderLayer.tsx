...
export const RenderLayer: FC<React.PropsWithChildren<IRenderLayerProps>> = (props) => {
  ...
  // 监听单元格保存状态变化，触发重绘
  const [cellSaveRenderTrigger, setCellSaveRenderTrigger] = useState(0);
  
  useEffect(() => {
    // 订阅状态变化
    const unsubscribe = useCellSaveStatus.subscribe(() => {
      // 状态变化时触发重新渲染
      setCellSaveRenderTrigger((prev) => prev + 1);
    });
